...
```
if ( number \> 100 ) { console.log ('Big') } else { console.log ('Small') } 
```
...
```
N = 100 // sets N to 100 N == 100 // false unless N has the value 100 
...
```
// Change the value of number and step through var number = -10 // example of 'and' if (number \>= 10 && number \< 100 ) { console.log('2 digit number') } else { console.log('1 digit number') } // example of 'or' if ( number \< 0 || number \>= 1000 ) { console.log('Invalid number') } else { console.log('OK') } 
```
